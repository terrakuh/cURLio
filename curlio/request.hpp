...
inline void Request::_lazy_init()
{
	...
	_data  = _owner;
	curl_easy_setopt(_owner->handle, CURLOPT_READFUNCTION, &Request::_send_callback);
	curl_easy_setopt(_owner->handle, CURLOPT_READDATA, this);
	// enable all encodings
	curl_easy_setopt(_owner->handle, CURLOPT_ACCEPT_ENCODING, "");
	_owner->request_finisher = [this] {
		if (_send_handler) {
